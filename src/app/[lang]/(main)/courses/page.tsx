...
function CourseCard({ course, isEnrolled, lang }: { course: Course, isEnrolled: boolean, lang: string }) {
    const href = `/${lang}${course.href}`
    return (
        <Card className="flex flex-col overflow-hidden hover:shadow-xl transition-shadow duration-300 relative">
            ...
            <CardHeader className="p-0">
                {course.imageUrl && <Link href={href} className="block aspect-video overflow-hidden group">
                    <Image
                        src={course.imageUrl}
                        alt={course.title}
